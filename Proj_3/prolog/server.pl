...
parse_input(getAIinput(Depth, Turns, Board), Coords):- getAIinput(Depth, Turns, Board, Coords).
parse_input(play(Board, Player, X, Y), [NewBoard, Cut]):- executePlay(Board, Player, X, Y, NewBoard, Cut).
parse_input(update_turns(Turns, Cut), Res):- updateTurnState(Turns, Cut, Res).
parse_input(get_player(Turns), Res):- getCurrentPlayer(Turns, Res).
